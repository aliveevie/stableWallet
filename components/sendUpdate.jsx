...
export function Send() {
  ...
  useEffect(() => {
    async function getMockOfferings() {
      // Extract unique PayIn currencies from mock data
      const payinCurrencies = [
        ...new Set(
          mockPFIs.flatMap((pfi) =>
            pfi.offerings.map((offering) => offering.payin.currencyCode)
          )
        ),
      ];
  
      // Flatten offerings from mock data and structure it for the component
      const offerings = mockPFIs.flatMap((pfi) =>
        pfi.offerings.map((offering) => ({
          data: offering,
          metadata: { from: pfi.pfiUri },
        }))
      );
  
      // Map PFIs to a simple allowlist format
      const pfiAllowlist = mockPFIs.map((pfi) => ({
        pfiName: pfi.pfiName,
        pfiUri: pfi.pfiUri,
      }));
  
      // Update state with mock data
      setPayIn(payinCurrencies); // Set PayIn currencies
      setOfferings(offerings); // Set offerings
      setState((prev) => ({ ...prev, pfiAllowlist })); // Set PFI allowlist
      setLoading(false); // Stop loading when data is populated
    }
  
    getMockOfferings(); // Initialize mock data
